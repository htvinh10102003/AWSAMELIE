        ...
        (payload) => {
          try {
            ...
            if (payload.new.key === 'global_notification') {
              ...
              const dismissedId = localStorage.getItem('dismissed_notif_id');
              if (notifData && notifData.enabled && String(notifData.id) !== dismissedId) setGlobalNotif(notifData);
      ...
      {globalNotif && (
        <div className="fixed inset-0 z-[200] flex items-center justify-center p-4 bg-slate-900/50 backdrop-blur-sm animate-in fade-in duration-300">
          <div className="bg-white rounded-3xl shadow-2xl w-full max-w-md overflow-hidden animate-in zoom-in-95 duration-300">
            <div className={`px-6 py-4 flex items-center justify-between border-b ${globalNotif.type === 'update' ? 'bg-blue-50 border-blue-100' : globalNotif.type === 'celebrate' ? 'bg-amber-50 border-amber-100' : 'bg-slate-50 border-slate-200'}`}>
              <div className="flex items-center gap-3">
                <div className={`p-2 rounded-xl bg-white shadow-sm ${globalNotif.type === 'update' ? 'text-blue-600' : globalNotif.type === 'celebrate' ? 'text-amber-500' : 'text-slate-600'}`}>
                  {globalNotif.type === 'update' ? <Settings size={20} /> : globalNotif.type === 'celebrate' ? <PartyPopper size={20} /> : <Info size={20} />}
                </div>
                <h3 className="font-black text-slate-800 text-lg">{globalNotif.title || 'Thông báo mới'}</h3>
              </div>
              <button onClick={() => setGlobalNotif(null)} className="p-1.5 text-slate-400 hover:text-slate-700 hover:bg-white rounded-lg transition-colors"><X size={20} /></button>
            </div>
            <div className="p-6">
              <div className="text-slate-600 text-sm font-medium leading-relaxed whitespace-pre-wrap mb-8">{globalNotif.content}</div>
              <div className="flex flex-col sm:flex-row items-center justify-between gap-4">
                <label className="flex items-center gap-2 text-sm text-slate-500 font-medium cursor-pointer hover:text-slate-800 transition">
                  <input type="checkbox" className="w-4 h-4 rounded border-slate-300 text-blue-600 cursor-pointer" checked={dontShowAgain} onChange={(e) => setDontShowAgain(e.target.checked)} />
                  Tắt thông báo này
                </label>
                <button onClick={handleCloseGlobalNotif} className="w-full sm:w-auto px-6 py-2.5 bg-slate-900 text-white font-bold rounded-xl shadow-md hover:bg-slate-800 transition-colors">Đã hiểu</button>
              </div>
            </div>
          </div>
        </div>
