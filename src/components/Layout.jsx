...
export default function Layout() {
  ...
  useEffect(() => {
    ...
    const fetchThemeConfig = async () => {
      const { data } = await supabase.from('system_configs').select('key, value').in('key', ['theme_christmas', 'theme_tet']);
      if (data) {
        data.forEach(item => {
  ...
  return (
    <>
      ...
      <div className={`flex h-screen font-sans antialiased text-gray-800 tracking-normal selection:bg-blue-500/20 selection:text-blue-700 transition-colors duration-1000 ${
        isTet ? 'bg-gradient-to-br from-red-50 via-orange-50/40 to-yellow-50/40' : isXmas ? 'bg-gradient-to-br from-red-50 via-white to-green-50/40' : 'bg-gradient-to-br from-slate-50 via-white to-blue-50/30'
      }`}>
        ...
        <div
          ...
          } ${isTet ? 'bg-white/90 tet-neon-border' : isXmas ? 'bg-white/80 xmas-neon-border' : 'bg-white/70 border-r border-white/30'}`}
        >
          ...
          <div className={`h-16 flex items-center justify-between px-4 relative backdrop-blur-md rounded-br-2xl whitespace-nowrap transition-colors ${
            isTet ? 'bg-red-50/80 border-b border-yellow-300/50' : isXmas ? 'bg-red-50/50 border-b border-red-200/50' : 'bg-white/50 border-b border-white/20'
          }`}>
            <div className="flex items-center">
              <div className={`p-1.5 rounded-xl shrink-0 transition-colors ${isTet ? 'bg-red-600 shadow-yellow-500/40 shadow-lg' : isXmas ? 'bg-red-600 shadow-red-500/30 shadow-lg' : 'bg-blue-600 shadow-blue-500/20 shadow-lg'}`}>
                <PackageSearch className="text-white" size={22} />
              </div>
              <h1 className={`text-xl font-black tracking-tight ml-3 transition-all duration-300 flex items-center gap-1 ${isTet || isXmas ? 'text-red-700' : 'text-gray-900'} ${sidebarExpanded ? 'opacity-100 max-w-[200px]' : 'opacity-0 max-w-0'}`}>
                Amelie<span className={`${isTet ? 'text-yellow-600' : isXmas ? 'text-green-600' : 'text-blue-600'} font-semibold`}>WMS</span> 
                {isTet ? <span className="animate-pulse drop-shadow-sm ml-1 text-sm">🧧</span> : isXmas ? <span className="animate-pulse ml-1 text-sm">🎄</span> : ''}
              </h1>
            </div>
            ...
            >
              {isSidebarPinned ? <PinOff size={14} className={isTet || isXmas ? 'text-red-400' : 'text-gray-500'} /> : <Pin size={14} className={isTet || isXmas ? 'text-red-400' : 'text-gray-500'} />}
            </button>
          </div>
          ...
          <div className="flex-1 overflow-y-auto overflow-x-hidden py-6 scrollbar-thin scrollbar-thumb-gray-200 scrollbar-track-transparent">
            <div className={`px-5 mb-3 text-[11px] font-bold uppercase tracking-widest transition-all duration-300 whitespace-nowrap ${isTet || isXmas ? 'text-red-400/80' : 'text-gray-400'} ${sidebarExpanded ? 'opacity-100 h-auto' : 'opacity-0 h-0 overflow-hidden'}`}>
              Báo cáo & Vận hành
            </div>
            ...
            <nav className="space-y-1.5 px-3 mb-8 relative">
              {reportMenus.map((item) => {
                ...
                const iconActiveColor = isTet || isXmas ? 'text-red-600' : 'text-blue-600';
                const iconHoverColor = isTet ? 'text-yellow-500' : isXmas ? 'text-green-500' : 'text-blue-500';
                ...
                if (item.id === 'dashboard') {
                  ...
                  return (
                    <div key={item.id} className="space-y-1.5">
                      <button onClick={() => setIsDashboardOpen(!isDashboardOpen)} className={`w-full flex items-center px-4 py-3 rounded-2xl group cursor-pointer overflow-hidden whitespace-nowrap ${isChildActive && !isDashboardOpen ? activeParentClass : `${wiggleClass} font-medium text-gray-600`}`}>
                      ...
                      {sidebarExpanded && isDashboardOpen && (
                        <div className={`mt-1 mb-2 ml-6 pl-3 border-l-2 flex flex-col gap-1 overflow-hidden animate-in slide-in-from-top-2 duration-200 whitespace-nowrap ${isTet || isXmas ? 'border-red-200/50' : 'border-slate-200/60'}`}>
                ...
                if (item.id === 'print_orders') {
                  ...
                  return (
                    <div key={item.id} className="space-y-1.5">
                      <button onClick={() => setIsPrintOrdersOpen(!isPrintOrdersOpen)} className={`w-full flex items-center px-4 py-3 rounded-2xl group cursor-pointer overflow-hidden whitespace-nowrap ${isChildActive && !isPrintOrdersOpen ? activeParentClass : `${wiggleClass} font-medium text-gray-600`}`}>
                        <div className="flex items-center shrink-0">
                          <Icon size={20} strokeWidth={2} className={`transition-colors ${isChildActive ? iconActiveColor : `text-gray-400 group-hover:${iconHoverColor}`}`} />
                        </div>
                        <div className={`flex items-center justify-between w-full ml-3 transition-all duration-300 ${sidebarExpanded ? 'opacity-100 max-w-[200px]' : 'opacity-0 max-w-0'}`}>
                      ...
                      {sidebarExpanded && isPrintOrdersOpen && (
                        <div className={`mt-1 mb-2 ml-6 pl-3 border-l-2 flex flex-col gap-1 overflow-hidden animate-in slide-in-from-top-2 duration-200 whitespace-nowrap ${isTet || isXmas ? 'border-red-200/50' : 'border-slate-200/60'}`}>
                          ...
                          <Link to="/in-don-spx" className={`flex items-center gap-3 px-3 py-2 rounded-xl ${wiggleClass} ${location.pathname === '/in-don-spx' ? (isTet ? 'bg-orange-50 text-orange-600 font-bold border-yellow-200/50' : 'bg-orange-50 text-orange-600 font-bold') : 'text-gray-500 font-medium text-sm'}`}>
                ...
                if (item.id === 'packing') {
                  ...
                  return (
                    <div key={item.id} className="space-y-1.5">
                      <button onClick={() => setIsPackingOpen(!isPackingOpen)} className={`w-full flex items-center px-4 py-3 rounded-2xl group cursor-pointer overflow-hidden whitespace-nowrap ${isChildActive && !isPackingOpen ? activeParentClass : `${wiggleClass} font-medium text-gray-600`}`}>
                        <div className="flex items-center shrink-0">
                          <Icon size={20} strokeWidth={2} className={`transition-colors ${isChildActive ? iconActiveColor : `text-gray-400 group-hover:${iconHoverColor}`}`} />
                        </div>
                        <div className={`flex items-center justify-between w-full ml-3 transition-all duration-300 ${sidebarExpanded ? 'opacity-100 max-w-[200px]' : 'opacity-0 max-w-0'}`}>
                ...
                if (item.id === 'return_orders') {
                  ...
                  return (
                    <div key={item.id} className="space-y-1.5">
                      <button onClick={() => setIsReturnOrdersOpen(!isReturnOrdersOpen)} className={`w-full flex items-center px-4 py-3 rounded-2xl group cursor-pointer overflow-hidden whitespace-nowrap ${isChildActive && !isReturnOrdersOpen ? activeParentClass : `${wiggleClass} font-medium text-gray-600`}`}>
                        <div className="flex items-center shrink-0">
                          <Icon size={20} strokeWidth={2} className={`transition-colors ${isChildActive ? iconActiveColor : `text-gray-400 group-hover:${iconHoverColor}`}`} />
                        </div>
                        <div className={`flex items-center justify-between w-full ml-3 transition-all duration-300 ${sidebarExpanded ? 'opacity-100 max-w-[200px]' : 'opacity-0 max-w-0'}`}>
                ...
                if (item.id === 'inventory_check') {
                  ...
                  return (
                    <div key={item.id} className="space-y-1.5">
                      <button onClick={() => setIsInventoryCheckOpen(!isInventoryCheckOpen)} className={`w-full flex items-center px-4 py-3 rounded-2xl group cursor-pointer overflow-hidden whitespace-nowrap ${isChildActive && !isInventoryCheckOpen ? activeParentClass : `${wiggleClass} font-medium text-gray-600`}`}>
                        <div className="flex items-center shrink-0">
                          <Icon size={20} strokeWidth={2} className={`transition-colors ${isChildActive ? iconActiveColor : `text-gray-400 group-hover:${iconHoverColor}`}`} />
                        </div>
                        <div className={`flex items-center justify-between w-full ml-3 transition-all duration-300 ${sidebarExpanded ? 'opacity-100 max-w-[200px]' : 'opacity-0 max-w-0'}`}>
                ...
                if (item.id === 'inventory_report') {
                  ...
                  return (
                    <div key={item.id} className="space-y-1.5">
                      <button onClick={() => setIsInventoryReportOpen(!isInventoryReportOpen)} className={`w-full flex items-center px-4 py-3 rounded-2xl group cursor-pointer overflow-hidden whitespace-nowrap ${isChildActive && !isInventoryReportOpen ? activeParentClass : `${wiggleClass} font-medium text-gray-600`}`}>
                        <div className="flex items-center shrink-0">
                          <Icon size={20} strokeWidth={2} className={`transition-colors ${isChildActive ? iconActiveColor : `text-gray-400 group-hover:${iconHoverColor}`}`} />
                        </div>
                        <div className={`flex items-center justify-between w-full ml-3 transition-all duration-300 ${sidebarExpanded ? 'opacity-100 max-w-[200px]' : 'opacity-0 max-w-0'}`}>
            ...
            {isAdmin && (
              <>
                ...
                <nav className="space-y-1.5 px-3">
                  <Link to="/admin" className={`flex items-center px-4 py-3 rounded-2xl group overflow-hidden whitespace-nowrap ${isTet ? 'hover-tet-gold transition-all duration-300 border border-transparent' : isXmas ? 'hover-xmas-wiggle' : ''} ${location.pathname === '/admin' ? (isTet ? 'bg-red-600/90 backdrop-blur-md text-white font-semibold shadow-lg shadow-yellow-500/20' : isXmas ? 'bg-red-600/90 backdrop-blur-md text-white font-semibold shadow-lg shadow-red-500/20' : 'bg-blue-600/90 backdrop-blur-md text-white font-semibold shadow-lg shadow-blue-500/20') : 'hover:bg-white/60 hover:text-gray-900 font-medium text-gray-600'}`}>
                    <div className="flex items-center shrink-0">
                      <Settings size={20} strokeWidth={location.pathname === '/admin' ? 2.5 : 2} className={`transition-colors duration-200 ${location.pathname === '/admin' ? 'text-white' : `text-gray-400 group-hover:${isTet ? 'text-yellow-500' : isXmas ? 'text-green-500' : 'text-blue-500'}`}`} />
                    </div>
                  ...
                  <div className="pt-1">
                    <button onClick={() => setIsKpiMenuOpen(!isKpiMenuOpen)} className={`w-full flex items-center px-4 py-3 rounded-2xl group cursor-pointer overflow-hidden whitespace-nowrap ${isTet ? 'hover-tet-gold transition-all duration-300 border border-transparent' : isXmas ? 'hover-xmas-wiggle hover:bg-red-50' : ''} ${(location.pathname.includes('/quan-ly-kpi') || location.pathname.includes('/nhap-lieu-kpi')) && !isKpiMenuOpen ? (isTet || isXmas ? 'bg-red-50 text-red-600 border-red-200/50 shadow-sm' : 'bg-blue-50 text-blue-600') : 'hover:bg-white/60 hover:text-gray-900 font-medium text-gray-600'}`}>
                      <div className="flex items-center shrink-0">
                        <Target size={20} strokeWidth={2} className={`transition-colors ${(location.pathname.includes('/quan-ly-kpi') || location.pathname.includes('/nhap-lieu-kpi')) ? (isTet || isXmas ? 'text-red-600' : 'text-blue-600') : `text-gray-400 group-hover:${isTet ? 'text-yellow-500' : isXmas ? 'text-green-500' : 'text-blue-500'}`}`} />
                      </div>
                      <div className={`flex items-center justify-between w-full ml-3 transition-all duration-300 ${sidebarExpanded ? 'opacity-100 max-w-[200px]' : 'opacity-0 max-w-0'}`}>
                    ...
                    {sidebarExpanded && isKpiMenuOpen && (
                      <div className={`mt-1 mb-2 ml-6 pl-3 border-l-2 flex flex-col gap-1 overflow-hidden animate-in slide-in-from-top-2 duration-200 whitespace-nowrap ${isTet || isXmas ? 'border-red-200/50' : 'border-slate-200/60'}`}>
                        <Link to="/quan-ly-kpi" className={`flex items-center gap-3 px-3 py-2 rounded-xl ${isTet ? 'hover-tet-gold transition-all duration-300 border border-transparent' : isXmas ? 'hover-xmas-wiggle' : ''} ${location.pathname === '/quan-ly-kpi' ? (isTet ? 'bg-red-50 text-red-700 font-bold border-yellow-200/50 shadow-sm' : isXmas ? 'bg-red-50 text-red-700 font-bold' : 'bg-blue-50 text-blue-700 font-bold') : 'hover:bg-white/60 hover:text-gray-900 text-gray-500 font-medium text-sm'}`}>
                          <Settings2 size={16} className={location.pathname === '/quan-ly-kpi' ? (isTet || isXmas ? 'text-red-600' : 'text-blue-600') : 'text-gray-400 shrink-0'} />
                          <span className="text-sm">Cấu hình KPI</span>
                        </Link>
                        <Link to="/nhap-lieu-kpi" className={`flex items-center gap-3 px-3 py-2 rounded-xl ${isTet ? 'hover-tet-gold transition-all duration-300 border border-transparent' : isXmas ? 'hover-xmas-wiggle' : ''} ${location.pathname === '/nhap-lieu-kpi' ? (isTet ? 'bg-red-50 text-red-700 font-bold border-yellow-200/50 shadow-sm' : isXmas ? 'bg-red-50 text-red-700 font-bold' : 'bg-blue-50 text-blue-700 font-bold') : 'hover:bg-white/60 hover:text-gray-900 text-gray-500 font-medium text-sm'}`}>
                          <FileEdit size={16} className={location.pathname === '/nhap-lieu-kpi' ? (isTet || isXmas ? 'text-red-600' : 'text-blue-600') : 'text-gray-400 shrink-0'} />
                  ...
                  <div className="pt-1">
                    <button onClick={() => setIsAdjustMenuOpen(!isAdjustMenuOpen)} className={`w-full flex items-center px-4 py-3 rounded-2xl group cursor-pointer overflow-hidden whitespace-nowrap ${isTet ? 'hover-tet-gold transition-all duration-300 border border-transparent' : isXmas ? 'hover-xmas-wiggle hover:bg-red-50' : 'hover:bg-white/60'} hover:text-gray-900 font-medium text-gray-600`}>
                      <div className="flex items-center shrink-0">
                        <Wrench size={20} strokeWidth={2} className={`text-gray-400 transition-colors group-hover:${isTet ? 'text-yellow-500' : isXmas ? 'text-green-500' : 'text-blue-500'}`} />
                      </div>
                      <div className={`flex items-center justify-between w-full ml-3 transition-all duration-300 ${sidebarExpanded ? 'opacity-100 max-w-[200px]' : 'opacity-0 max-w-0'}`}>
                    ...
                    {sidebarExpanded && isAdjustMenuOpen && (
                      <div className={`mt-1 mb-2 ml-6 pl-3 border-l-2 flex flex-col gap-1 overflow-hidden animate-in slide-in-from-top-2 duration-200 whitespace-nowrap ${isTet || isXmas ? 'border-red-200/50' : 'border-slate-200/60'}`}>
                        {isOwner && (
                          <Link to="/cap-nhat-tinh-nang" className={`flex items-center gap-3 px-3 py-2 rounded-xl ${isTet ? 'hover-tet-gold transition-all duration-300 border border-transparent' : isXmas ? 'hover-xmas-wiggle' : ''} ${location.pathname === '/cap-nhat-tinh-nang' ? (isTet ? 'bg-red-50 text-red-700 font-bold border-yellow-200/50 shadow-sm' : isXmas ? 'bg-red-50 text-red-700 font-bold' : 'bg-blue-50 text-blue-700 font-bold') : 'hover:bg-white/60 hover:text-gray-900 text-gray-500 font-medium text-sm'}`}>
                            <ShieldAlert size={16} className={location.pathname === '/cap-nhat-tinh-nang' ? (isTet || isXmas ? 'text-red-600' : 'text-blue-600') : 'text-gray-400 shrink-0'} />
                            <span className="text-sm">Khóa tính năng</span>
                          </Link>
                        )}
                        <Link to="/cap-nhat-nguoi-dong-goi" className={`flex items-center gap-3 px-3 py-2 rounded-xl ${isTet ? 'hover-tet-gold transition-all duration-300 border border-transparent' : isXmas ? 'hover-xmas-wiggle' : ''} ${location.pathname === '/cap-nhat-nguoi-dong-goi' ? (isTet ? 'bg-red-50 text-red-700 font-bold border-yellow-200/50 shadow-sm' : isXmas ? 'bg-red-50 text-red-700 font-bold' : 'bg-blue-50 text-blue-700 font-bold') : 'hover:bg-white/60 hover:text-gray-900 text-gray-500 font-medium text-sm'}`}>
                          <UserCog size={16} className={location.pathname === '/cap-nhat-nguoi-dong-goi' ? (isTet || isXmas ? 'text-red-600' : 'text-blue-600') : 'text-gray-400 shrink-0'} />
                          <span className="text-sm">Người đóng gói</span>
                        </Link>
                        <Link to="/cap-nhat-lich-lam-viec" className={`flex items-center gap-3 px-3 py-2 rounded-xl ${isTet ? 'hover-tet-gold transition-all duration-300 border border-transparent' : isXmas ? 'hover-xmas-wiggle' : ''} ${location.pathname === '/cap-nhat-lich-lam-viec' ? (isTet ? 'bg-red-50 text-red-700 font-bold border-yellow-200/50 shadow-sm' : isXmas ? 'bg-red-50 text-red-700 font-bold' : 'bg-blue-50 text-blue-700 font-bold') : 'hover:bg-white/60 hover:text-gray-900 text-gray-500 font-medium text-sm'}`}>
                          <CalendarDays size={16} className={location.pathname === '/cap-nhat-lich-lam-viec' ? (isTet || isXmas ? 'text-red-600' : 'text-blue-600') : 'text-gray-400 shrink-0'} />
                          <span className="text-sm">Lịch làm việc</span>
                        </Link>
                        <Link to="/cap-nhat-san-pham" className={`flex items-center gap-3 px-3 py-2 rounded-xl ${isTet ? 'hover-tet-gold transition-all duration-300 border border-transparent' : isXmas ? 'hover-xmas-wiggle' : ''} ${location.pathname === '/cap-nhat-san-pham' ? (isTet ? 'bg-red-50 text-red-700 font-bold border-yellow-200/50 shadow-sm' : isXmas ? 'bg-red-50 text-red-700 font-bold' : 'bg-blue-50 text-blue-700 font-bold') : 'hover:bg-white/60 hover:text-gray-900 text-gray-500 font-medium text-sm'}`}>
                          <PackageSearch size={16} className={location.pathname === '/cap-nhat-san-pham' ? (isTet || isXmas ? 'text-red-600' : 'text-blue-600') : 'text-gray-400 shrink-0'} />
                          <span className="text-sm">Hiệu chỉnh sản phẩm</span>
                        </Link>
                        <Link to="/cap-nhat-so-do-kho" className={`flex items-center gap-3 px-3 py-2 rounded-xl ${isTet ? 'hover-tet-gold transition-all duration-300 border border-transparent' : isXmas ? 'hover-xmas-wiggle' : ''} ${location.pathname === '/cap-nhat-so-do-kho' ? (isTet ? 'bg-red-50 text-red-700 font-bold border-yellow-200/50 shadow-sm' : isXmas ? 'bg-red-50 text-red-700 font-bold' : 'bg-blue-50 text-blue-700 font-bold') : 'hover:bg-white/60 hover:text-gray-900 text-gray-500 font-medium text-sm'}`}>
                          <MapPin size={16} className={location.pathname === '/cap-nhat-so-do-kho' ? (isTet || isXmas ? 'text-red-600' : 'text-blue-600') : 'text-gray-400 shrink-0'} />
                          <span className="text-sm">Sơ đồ Kho hàng</span>
                        </Link>
                        <Link to="/cap-nhat-day-ke" className={`flex items-center gap-3 px-3 py-2 rounded-xl ${isTet ? 'hover-tet-gold transition-all duration-300 border border-transparent' : isXmas ? 'hover-xmas-wiggle' : ''} ${location.pathname === '/cap-nhat-day-ke' ? (isTet ? 'bg-red-50 text-red-700 font-bold border-yellow-200/50 shadow-sm' : isXmas ? 'bg-red-50 text-red-700 font-bold' : 'bg-blue-50 text-blue-700 font-bold') : 'hover:bg-white/60 hover:text-gray-900 text-gray-500 font-medium text-sm'}`}>
                          <LayoutGrid size={16} className={location.pathname === '/cap-nhat-day-ke' ? (isTet || isXmas ? 'text-red-600' : 'text-blue-600') : 'text-gray-400 shrink-0'} />
                          <span className="text-sm">Quy ước dãy kệ</span>
                        </Link>
                        <Link to="/cap-nhat-webhook" className={`flex items-center gap-3 px-3 py-2 rounded-xl ${isTet ? 'hover-tet-gold transition-all duration-300 border border-transparent' : isXmas ? 'hover-xmas-wiggle' : ''} ${location.pathname === '/cap-nhat-webhook' ? (isTet ? 'bg-red-50 text-red-700 font-bold border-yellow-200/50 shadow-sm' : isXmas ? 'bg-red-50 text-red-700 font-bold' : 'bg-blue-50 text-blue-700 font-bold') : 'hover:bg-white/60 hover:text-gray-900 text-gray-500 font-medium text-sm'}`}>
                          <Webhook size={16} className={location.pathname === '/cap-nhat-webhook' ? (isTet || isXmas ? 'text-red-600' : 'text-blue-600') : 'text-gray-400 shrink-0'} />
          ...
          <div className={`p-4 backdrop-blur-lg border-t rounded-tr-2xl overflow-hidden whitespace-nowrap transition-colors ${isTet ? 'bg-red-50/80 border-yellow-300/50' : isXmas ? 'bg-red-50/50 border-red-200/50' : 'bg-white/40 border-white/20'}`}>
            <div className={`flex items-center justify-between transition-all duration-300 ${!sidebarExpanded ? 'flex-col gap-2' : ''}`}>
              <div className={`flex items-center gap-3 ${sidebarExpanded ? 'max-w-[75%]' : ''}`}>
                ...
                <div className="relative">
                  ...
                  {isTet ? (
                    <div className="absolute -bottom-1 -right-2 text-xl drop-shadow-md z-10 pointer-events-none animate-bounce" style={{animationDuration: '3s'}}>
                      🌸
                    </div>
                  ) : isXmas ? (
                ...
                <div className={`flex flex-col transition-all duration-300 overflow-hidden ${sidebarExpanded ? 'opacity-100 max-w-[150px]' : 'opacity-0 max-w-0 hidden'}`}>
                  <span className={`text-sm font-bold leading-tight truncate capitalize ${isTet ? 'text-red-900' : 'text-gray-900'}`}>{displayName}</span>
                  <span className={`text-[10px] font-medium mt-0.5 truncate ${isTet ? 'text-yellow-700' : isXmas ? 'text-red-500' : 'text-gray-500'}`}>{isAdmin ? '🛡️ Admin' : '📦 Nhân viên'}</span>
                </div>
              </div>
              
              <button onClick={() => setShowLogoutModal(true)} title="Đăng xuất" className={`p-2 transition-colors rounded-xl cursor-pointer shrink-0 ${isTet ? 'text-red-500 hover:text-red-700 hover:bg-yellow-100/50' : isXmas ? 'text-red-400 hover:text-red-600 hover:bg-red-50' : 'text-gray-400 hover:text-red-500 hover:bg-white/60'} ${!sidebarExpanded ? 'p-1' : ''}`}>
        ...
        <div className="flex-1 overflow-auto relative bg-transparent">
          ...
          >
            <Menu size={20} className={isTet || isXmas ? 'text-red-700' : 'text-gray-700'} />
          </button>

          <div className={`absolute inset-0 pointer-events-none transition-colors duration-1000 ${isTet ? 'bg-gradient-to-b from-yellow-50/20 via-transparent to-red-50/10' : isXmas ? 'bg-gradient-to-b from-red-50/20 via-transparent to-green-50/10' : 'bg-gradient-to-b from-blue-50/20 via-white to-transparent'}`} />
          <div className={`absolute top-0 left-0 right-0 h-40 pointer-events-none transition-colors duration-1000 ${isTet ? 'bg-gradient-to-b from-red-100/40 to-transparent' : isXmas ? 'bg-gradient-to-b from-red-100/30 to-transparent' : 'bg-gradient-to-b from-blue-50/40 to-transparent'}`} />
